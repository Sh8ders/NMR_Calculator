...
from nmr_calculator.molecule import get_proton_environment_groups, prepare_molecule
...
def predict_proton_shift_for_group(mol: Mol, group: dict) -> dict:
    ...
    return {
        ...
        "prediction_method": "phase_4_rule_based",
        "notes": rule.notes,
    }
...
def get_shift_rule_for_group(mol: Mol, group: dict) -> ShiftRule:
    ...
    if "aldehyde proton" in environment_label:
        return ShiftRule(9.7, (9.0, 10.5), "medium", "aldehyde proton rule")

    if "aromatic proton" in environment_label:
        return ShiftRule(7.2, (6.0, 8.5), "medium", "aromatic proton rule")

    if "alkene proton" in environment_label:
        return ShiftRule(5.5, (4.5, 6.5), "medium", "vinylic proton rule")

    if "exchangeable proton" in environment_label:
        atom_symbols = set(group.get("atom_symbols", []))
        if "O" in atom_symbols and any(_is_carboxylic_acid_oxygen(mol.GetAtomWithIdx(atom_index)) for atom_index in atom_indices):
            return ShiftRule(
                11.5,
                (10.0, 13.0),
                "low",
                "carboxylic acid exchangeable proton rule",
            )
        return ShiftRule(
            2.5,
            (0.5, 5.5),
            "low",
            "exchangeable OH/NH/SH proton rule",
        )

    if any(is_adjacent_to_carbonyl(mol, atom_index) for atom_index in atom_indices):
        return ShiftRule(
            2.2,
            (2.0, 3.0),
            "medium",
            "alkyl proton alpha to carbonyl rule",
        )
    ...
    if "heteroatom-adjacent alkyl proton" in environment_label:
        return ShiftRule(
            ...
            (3.0, 4.5),
            "medium",
            "alkyl proton adjacent to heteroatom rule",
        )

    if any(is_adjacent_to_aromatic_or_alkene(mol, atom_index) for atom_index in atom_indices):
        return ShiftRule(
            2.2,
            (1.7, 2.7),
            "low",
            "alkyl proton adjacent to alkene or aromatic pi system rule",
        )

    if any(_is_terminal_alkyne_carbon(mol.GetAtomWithIdx(atom_index)) for atom_index in atom_indices):
        return ShiftRule(2.5, (2.0, 3.0), "low", "terminal alkyne proton rule")

    if environment_label.startswith("alkyl CH"):
        return ShiftRule(1.2, (0.8, 1.8), "medium", "simple alkyl proton rule")

    return ShiftRule(1.5, (0.5, 12.0), "low", "fallback unknown environment rule")
